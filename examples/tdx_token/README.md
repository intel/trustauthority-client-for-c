...
```
- When successfully built, running `docker image ls -a` includes `taas/tdx_token:v1.1.1`.
...
```
- When successfully built, running `docker image ls -a` includes `taas/azure_tdx_token:v1.1.1`.
...
- Use docker to run the TDX Token example...
    ...
    sudo docker run -it --rm --device=/dev/tdx_guest --env-file tdx_token.env --group-add $(getent group <user-group> | cut -d: -f3)  taas/intel_tdx_token:v1.1.1
    ```
...
- Use docker to run the Azure TDX Token example...
    ...
    sudo docker run -it --rm --device=/dev/tpm0 --device=/dev/tpmrm0 --env-file tdx_token.env --group-add $(getent group tss | cut -d: -f3) taas/azure_tdx_token:v1.1.1
    ```
