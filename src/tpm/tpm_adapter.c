...
static int get_quote(ESYS_CONTEXT *ctx,
                     uint32_t ak_handle,
...
static int get_pcrs(ESYS_CONTEXT *ctx,
                    TPML_PCR_SELECTION *pcr_selection,
                    uint8_t **flattened_pcrs,
                    size_t *flattened_pcrs_len);

static TRUST_AUTHORITY_STATUS read_sysfile_base64(const char *path, uint8_t **base64_buffer, size_t *buffer_len);

bool pcr_parse_selections(const char *arg, TPML_PCR_SELECTION *pcr_select, void *forwards); 
                     ...
                     uint32_t user_data_len)
{
	...
	if (nonce_data != NULL || user_data != NULL)
	{
        ...
        if (NULL == mdctx)
		{
			status = STATUS_TPM_ERROR_BASE | STATUS_INVOCATION_ERROR;
			goto ERROR;
		}
